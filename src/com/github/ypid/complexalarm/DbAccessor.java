/****************************************************************************
 * Copyright 2010 kraigs.android@gmail.com Licensed under the Apache License,
...
//
//        long id = rwDb.insert(DbHelper.DB_TABLE_ALARMS, null,
//                info.contentValues());
//        if (id < 0) {
...
//    }
    ...
    public boolean deleteAlarm(long alarmId) {
        int count = rDb.delete(DbHelper.DB_TABLE_ALARMS,
                DbHelper.ALARMS_COL_AID + " = " + alarmId, null);
        // This may or may not exist. We don't care about the return value.
        rDb.delete(DbHelper.DB_TABLE_SETTINGS, DbHelper.DB_TABLE_SETTINGS + " = "
                + alarmId, null);
        return count > 0;
    }
    ...
    public List<Long> getAllAlarms() {
        ...
        Cursor cursor = rDb.query(DbHelper.DB_TABLE_ALARMS,
                new String[] { DbHelper.ALARMS_COL_AID }, null, null, null,
                null, null);
        while (cursor.moveToNext()) {
            long alarmId = cursor.getLong(cursor
                    .getColumnIndex(DbHelper.ALARMS_COL_AID));
...
//                DbHelper.ALARMS_COL_AID + " = " + alarmId, null) == 1;
//    }

    public boolean writeAlarmSettings(long alarmId, AlarmSettings settings) {
        Cursor cursor = rDb.query(DbHelper.DB_TABLE_SETTINGS,
                new String[] { DbHelper.SETTINGS_COL_SID },
                DbHelper.SETTINGS_COL_SID + " = " + alarmId, null, null, null,
                null);

        boolean success = false;
        if (cursor.getCount() < 1) {
            success = rwDb.insert(DbHelper.DB_TABLE_SETTINGS, null,
                    settings.contentValues(alarmId)) >= 0;
        } else {
            success = rwDb.update(DbHelper.DB_TABLE_SETTINGS,
                    settings.contentValues(alarmId), DbHelper.SETTINGS_COL_SID
                            + " = " + alarmId, null) == 1;
        }
        cursor.close();
        return success;
    ...
    public AlarmSettings readAlarmSettings(long alarmId) {
        Cursor cursor = rDb.query(DbHelper.DB_TABLE_SETTINGS,
                AlarmSettings.contentColumns(), DbHelper.SETTINGS_COL_SID
                        + " = " + alarmId, null, null, null, null);
